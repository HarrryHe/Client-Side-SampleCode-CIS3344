"use strict";
function MakePhotoList({ photoList = [{}], title = "Default Photo List" }) {
    function MakePhoto ({
    ...
    var photoListComp = document.createElement("div");
    photoListComp.classList.add("photoList");
    photoListComp.innerHTML = `<h2>${title}</h2>`

    for (var photoObj of photoList) {
        photoListComp.appendChild(MakePhoto(photoObj));
    }

    return photoListComp;
}